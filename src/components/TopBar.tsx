    ...
    <header className="flex flex-col gap-3 rounded-lg bg-card p-3 md:p-4 shadow-sm">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1 md:gap-2 flex-wrap justify-end">
          <Select value={String(refreshInterval)} onValueChange={(value) => onIntervalChange(Number(value))}>
      ...
      <div className="flex items-center gap-3 overflow-x-auto pb-1 -mb-1 scrollbar-thin">
        {wifi.map((board) => (
          <div key={board.name} className="flex items-center gap-1 text-[10px] md:text-xs text-muted-foreground whitespace-nowrap shrink-0">
            <Wifi className="h-3 w-3 md:h-3.5 md:w-3.5" />
            <span>{board.name}</span>
            <RssiIndicator rssi={board.rssi} />
            <span>{board.rssi === null ? '--' : `${board.rssi} dBm`}</span>
          </div>
        ))}
      </div>
    </header>
